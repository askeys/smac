...
Tree *build_kdtree(double *reference, int N, int D, int *index, 
  ...
  if ( (treeptr = (Tree *) malloc(sizeof(Tree))) == NULL )
    printf("Error allocating memory for kD-Tree\n");
...
#ifdef DEBUG_BUILD_TREE
    printf("Created leaf     node, (dim=%d) (value: ", offset);
    for (i=0; i < D; i++)
      printf(" %f",nodeptr->pt[i]);
    printf(" )\n");
#endif
...
#ifdef DEBUG_BUILD_TREE
  printf("Created internal node, (dim=%d) (value: ", offset);
  for (i=0; i < D; i++)
    printf(" %f",nodeptr->pt[i]);
  printf(" )\n");
#endif
...
void run_queries( Node *pVertex, double *model, int M, int D, 
  ...
  for (i=0; i < M; i++) {
    ...
    printf("Running Query (%d/%d) (value: ",
	      i+1, M);
    for (j=0; j < D ; j++)
      printf(" %f", model[ M*j+i]);
    printf(" )\n");
#endif
...
#ifdef DEBUG_RUN_QUERIES
    printf("Distance to closest point is %f\n",distance[i]);
#endif
...
void display_tree(Node *nodeptr,int D) {
  ...
  printf("Node: (dim = %d) (idx= %u) (value = ", nodeptr->orientation,
	    nodeptr->index);
  for (i=0; i <D; i++)
    printf("%f\t",nodeptr->pt[i]);
  printf(")\n");
...
void run_range_search( Node *pVertex, double *model, int M, int D, 
  ...
  for (i=0; i < M; i++) {
    ...
    printf("Running Search (%d/%d) (value: ",
	      i+1, M);
    for (j=0; j < D ; j++)
      printf(" %f", model[ M*j+i]);
    printf(" )\n");
